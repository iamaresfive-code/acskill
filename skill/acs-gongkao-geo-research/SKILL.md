...
| `institution-deep-dive` | 用户指定一个机构、品牌或老师/IP | 只调查该指定主体，完成实体图谱、泛词表现、外部验证和空白分析；除非用户另行要求，不自动补充其他机构 |
| `institution-comparison` | 用户指定两个及以上主体进行比较 | 只比较用户点名的全部主体，使用统一题池、窗口和评分口径；除非用户另行要求，不新增对标主体 |
...
| `institution_names` | 机构、品牌、老师/IP，可为零个、一个或多个 | 只认用户当前请求中明确点名的主体。零个则调查整个地区并自动发现；一个或多个则只调查这些指定主体，除非用户明确要求增加对标；指定主体无论证据强弱都保留 |
...
- 用户明确点名一个或多个机构时，才把这些主体记为 `Specified`，并关闭地区候选自动发现：只调查用户点名的集合。点名主体不得因分数低而删除；除非用户再次明确要求增加对标，否则不得加入任何其他机构。
...
完成标志：五维分数可复算、等级正确、置信度有依据；地区模式只含自动发现主体，指定模式只含用户点名主体；用户明确要求对标时，新增对标主体与指定主体已区分。
...
6. 若用户点名了机构，检查报告和 `scores.csv` 是否只包含点名主体；存在 `Benchmark` 时必须能回溯到用户明确提出的增加对标要求。
7. 检查报告明确观察日期、研究范围、真实采样方式、证据覆盖和局限。
8. 生成 `report.pdf`，渲染全部页面并完成视觉检查；未通过不得交付。
9. 若校验仍有 error，不得称完整报告已完成；warning 必须人工复核并在交付中说明。
