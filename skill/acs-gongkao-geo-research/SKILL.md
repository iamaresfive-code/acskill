...
- 用户要求在地区排名中加入自有机构时，仍使用 `regional-landscape`：继续调查整个地区，同时把该机构作为 `Specified` 强制纳入评分和重点分析。即使 Generic Query 未发现它，也不得从报告删除。
- 强制纳入不等于加分。其 Generic 命中为零时如实记录，并解释“未被本次泛词样本发现”不等于机构不存在、经营失败或教学能力弱。
...
- 不因自有机构被强制纳入而增加分数、虚构 Generic 命中或调整排名；其未被发现时如实呈现并分析优化空间。
...
9. 生成 `report.pdf`，渲染全部页面并完成视觉检查；未通过不得交付。
10. 若校验仍有 error，不得称完整报告已完成；warning 必须人工复核并在交付中说明。
