...
TRUE={"1","true","yes","y","是"}
OUT_FIELDS=["entity_id","canonical_name","entity_type","market_scope","market_role","answer_cells","mentioned_answers","nomination_rate","top3_answers","top3_rate","first_mention_answers","first_mention_rate","cited_answers","citation_rate","engines_sampled","engines_mentioned","cross_model_consistency"]
...
def rjsonl(p:Path):
    ...
    with p.open("r",encoding="utf-8") as f:
        for n,line in enumerate(f,1):
            if line.strip():
                obj=json.loads(line);obj["_line"]=n;out.append(obj)
    return out
...
def compute(run:Path):
    ...
    for eid,u in umap.items():
        if u.get("universe_status")!="included":continue
        et=(u.get("entity_type") or "").lower();target="ip" if et in {"teacher","ip","expert"} or u.get("market_role")=="expert-ip" else "institution"
        ...
        rows.append({
            "entity_id":eid,"canonical_name":u.get("canonical_name"),"entity_type":u.get("entity_type"),"market_scope":u.get("market_scope"),"market_role":u.get("market_role"),
    ...
    rows.sort(key=lambda r:(r["entity_type"] in {"teacher","ip","expert"},-r["nomination_rate"],-r["top3_rate"],r["canonical_name"] or ""))
