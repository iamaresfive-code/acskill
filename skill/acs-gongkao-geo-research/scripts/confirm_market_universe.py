...
def confirm(run:Path,approved_by:str="user"):
    ...
    for i,r in enumerate(rows,2):
        name=(r.get("canonical_name") or "").strip() or f"row-{i}";scope=(r.get("market_scope") or "").strip();role=(r.get("market_role") or "").strip();status=(r.get("universe_status") or "").strip();basis=(r.get("salience_basis") or "").strip()
