...
def validate_measurement(run:Path,meta:dict,issues:list[Issue],ids:set,mode:str):
    ...
    queries,_=rcsv(run/"queries.csv",issues,"queries",qreq);answers=rjsonl(run/"ai_answers.jsonl",issues,"ai-answers");mentions,_=rcsv(run/"ai_mentions.csv",issues,"ai-mentions",mreq);serp,_=rcsv(run/"serp_results.csv",issues,"serp-results",sreq);serp_mentions,_=rcsv(run/"serp_mentions.csv",issues,"serp-mentions",smreq);page_mentions,_=rcsv(run/"page_mentions.csv",issues,"page-mentions",pmreq);rcsv(run/"evidence.csv",issues,"evidence",ereq);rechecks,_=rcsv(run/"rechecks.csv",issues,"rechecks",rreq);metrics,_=rcsv(run/"ai_metrics.csv",issues,"ai-metrics",{"entity_id","raw_mention_rate","nomination_rate","top3_rate","first_mention_rate","citation_rate","answer_cells","engines_sampled","engine_coverage_rate","cross_model_consistency"});emergent,_=rcsv(run/"ai_emergent_entities.csv",issues,"ai-emergent-entities",xreq)
    ...
    if expected_mode not in VALID_CONTEXT_MODES:issues.append(Issue("error","answer-context-mode-meta","answer_context_mode_expected 必须 native/engine-native-search/external-search-augmented"))
    repeats=intnum(meta.get("repeat_runs_expected"))
    if repeats is None or repeats<1:issues.append(Issue("error","repeat-runs-meta","repeat_runs_expected 必须为 >=1 的整数"));repeats=1
    if profile=="release" and repeats<3:issues.append(Issue("error","release-repeat-minimum","release profile 至少要求每 query×engine 独立采样 3 次"))
    ...
    qmap={q.get("query_id"):q for q in queries};answer_ids=set();answers_by_query=defaultdict(list);cell_keys=set();context_ids=set()
    for a in answers:
        ...
        if sr==-1:issues.append(Issue("error","sample-run",f"{aid} sample_run 必须为 >=1 的整数"))
        ...
        if key in cell_keys:issues.append(Issue("error","answer-cell-duplicate",f"重复 Answer Cell：query/engine/model/sample_run={key}"))
    ...
    for m in mentions:
        ...
        if eid not in valid_raw_entity_ids:issues.append(Issue("error","mention-entity",f"{mid} 引用不存在 entity_id；若为新主体，先写 ai_emergent_entities.csv"));continue
