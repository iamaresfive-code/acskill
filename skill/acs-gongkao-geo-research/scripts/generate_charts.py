...
Rules:
- Customer charts are Chinese-first. Missing CJK font is a hard error, not an English/number fallback.
...
def generate(run:Path):
    ...
    # AI 可见度 × GEO 资产基础：每个现实主体只画一个点。
    asset_by={r.get("entity_id"):r for r in assets};best_by_entity={}
    for r in metrics:
        eid=r.get("entity_id");a=asset_by.get(eid) or {};x=fnum(a.get("asset_readiness"));y=fnum(r.get("nomination_rate"))
    ...
    if points:
        xs=[p["x"] for p in points];ys=[p["y"] for p in points];mx=statistics.median(xs);my=statistics.median(ys)
        fig,ax=plt.subplots(figsize=(6.7,4.4),dpi=180);ax.scatter(xs,ys,s=30);ax.axvline(mx,linestyle="--",linewidth=0.8);ax.axhline(my,linestyle="--",linewidth=0.8)
        top=sorted(range(len(points)),key=lambda i:(points[i]["y"],points[i]["x"]),reverse=True)[:12]
        for i in top:
            p=points[i];ax.annotate(p["name"],(p["x"],p["y"]),xytext=(4,4),textcoords="offset points",fontsize=7)
        ax.set_xlabel("GEO 资产基础得分");ax.set_ylabel("AI 提名率（%）");ax.set_title("AI 可见度 × GEO 资产基础（虚线为本轮中位数）");ax.set_xlim(0,105);ax.set_ylim(bottom=0)
        p=out/"market-universe.png";_save(fig,plt,p);files.append(p.name)

    def visibility(rows,filename,title):
        rows=[r for r in rows if fnum(r.get("nomination_rate")) is not None];rows=sorted(rows,key=lambda r:fnum(r.get("nomination_rate")) or 0,reverse=True)[:12]
        if not rows:return
        labels=[r.get("canonical_name","") for r in rows];values=[100*(fnum(r.get("nomination_rate")) or 0) for r in rows]
        fig,ax=plt.subplots(figsize=(6.7,max(3.2,0.34*len(rows)+1.4)),dpi=180);bars=ax.barh(list(reversed(labels)),list(reversed(values)));_bar_labels(ax,bars,list(reversed(values)))
    ...
    if scored:
        labels=[r.get("canonical_name") or r.get("institution") or r.get("entity_id") for r,_ in scored];values=[s for _,s in scored];extras=[f" / {r.get('asset_tier')}" if r.get("asset_tier") else "" for r,_ in scored]
        fig,ax=plt.subplots(figsize=(6.7,max(3.2,0.34*len(scored)+1.5)),dpi=180);bars=ax.barh(list(reversed(labels)),list(reversed(values)));_bar_labels(ax,bars,list(reversed(values)),suffix="",extras=list(reversed(extras)))
        ax.set_xlim(0,112);ax.set_xlabel("GEO 资产基础得分（100分制）");ax.set_title("GEO 资产基础排名（仅展示可评分主体）")
    ...
    if ranked:
        ...
        fig,ax=plt.subplots(figsize=(6.7,max(3.4,0.42*len(ranked)+1.4)),dpi=180);bars=ax.barh(list(reversed(labels)),list(reversed(values)));_bar_labels(ax,bars,list(reversed(values)),suffix="")
    ...
    payload={"files":files,"cjk_font_available":True,"cjk_font":cjk_font,"customer_chart_language":"zh-CN","generated_from":{"ai_metrics":len(metrics),"asset_scores":len(assets),"concept_rows":len(concepts),"opportunity_matrix_entities":len(points)}}
