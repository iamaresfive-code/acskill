#!/usr/bin/env python3
"""Generate GEO v2.1.1 SVG charts from current-run CSV/JSON only."""
...
def recall_by_entity(queries,results):
    qmap={q.get("query_id",""):q for q in queries};valid={qid for qid,q in qmap.items() if q.get("query_type","").lower()=="generic" and q.get("query_purpose","").lower()=="measurement" and q.get("measurement_target","").lower()=="institution" and q.get("status","").lower()=="sampled"};hits=defaultdict(set)
    for r in results:
        if r.get("matched","").lower() in TRUE and r.get("counts_as_measurement_hit","").lower() in TRUE and r.get("query_id","") in valid and r.get("entity_id",""):hits[r["entity_id"]].add(r["query_id"])
    den=len(valid);return {eid:(len(qs)/den if den else 0.0) for eid,qs in hits.items()},den

def authority_indices(scores,evidence,relations,observation:date):
    by=defaultdict(list);rel_by=defaultdict(list)
    ...
    for s in scores:
        eid=s.get("entity_id","");rows=[e for e in by[eid] if e.get("counting_scope","").lower()!="ignored"];g=sum(grade.get(e.get("source_grade","").upper(),0) for e in rows)/len(rows) if rows else 0
        domains={urlparse(e.get("source_url","")).netloc.lower().removeprefix("www.") for e in rows if e.get("independent","").lower() in TRUE and e.get("source_url")};div={0:0,1:.35,2:.6,3:.8}.get(len(domains),1.0)
        keyrels=[r for r in rel_by[eid] if r.get("relation_type","") in {"operated-by","brand-of","teaches-at","founded-by","offers","located-in"}];cross=sum(1 for r in keyrels if len(split(r.get("evidence_ids","")))>=2)/len(keyrels) if keyrels else 0
        fresh_rows=[e for e in rows if parse_date(e.get("published_date",""))];fresh=sum((observation-parse_date(e.get("published_date",""))).days<=365 for e in fresh_rows)/len(fresh_rows) if fresh_rows else 0
        out[eid]=round(100*(.35*g+.25*div+.20*cross+.10*fresh+.10*(1 if rows else 0)),1)
    return out

def svg_wrap(width,height,body,title):
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="{esc(title)}" style="width:100%;display:block"><rect width="100%" height="100%" fill="#fff"/><style>text{{font-family:'Noto Sans CJK SC','PingFang SC','Microsoft YaHei',sans-serif;fill:#182638}} .muted{{fill:#6b778c}} .axis{{stroke:#b8c4d4;stroke-width:1}} .grid{{stroke:#e5eaf1;stroke-width:1}} .bar{{fill:#244f7d}} .label{{font-size:13px}} .small{{font-size:11px}} .title{{font-size:20px;font-weight:700}}</style><text x="24" y="30" class="title">{esc(title)}</text>{body}</svg>'''
...
def ranking_svg(scores):
    ...
    for i,r in enumerate(rows):
        y=top+i*rowh;value=num(r.get("total"));bw=(w-left-120)*value/100;body.append(f'<text x="{left-10}" y="{y+17}" text-anchor="end" class="label">{esc(r.get("institution",""))}</text><rect x="{left}" y="{y+3}" width="{bw:.1f}" height="18" rx="3" class="bar"/><text x="{left+bw+8:.1f}" y="{y+17}" class="label">{value:g} / {esc(r.get("tier",""))}</text>')
    body.append(f'<text x="24" y="{h-20}" class="small muted">GEO观察指数衡量公开网络与AI可利用资产结构，不代表教学水平、市场份额或通过率。</text>');return svg_wrap(w,h,"".join(body),"GEO综合排名")

def _spread_labels(points,top=78,bottom=470,min_gap=18):
    ordered=sorted(points,key=lambda p:p[2]);placed=[]
    for idx,x,y in ordered:
        ly=max(top,min(bottom,y-7))
        if placed and ly-placed[-1][3]<min_gap:ly=placed[-1][3]+min_gap
        placed.append([idx,x,y,ly])
    if placed and placed[-1][3]>bottom:
        shift=placed[-1][3]-bottom
        for p in placed:p[3]-=shift
    return {idx:ly for idx,_,_,ly in placed}
...
def matrix_svg(scores,recall,authority):
    ...
    body=[f'<line x1="{x0}" y1="{y0+ph}" x2="{x0+pw}" y2="{y0+ph}" class="axis"/><line x1="{x0}" y1="{y0}" x2="{x0}" y2="{y0+ph}" class="axis"/>',f'<line x1="{x0+pw*mx:.1f}" y1="{y0}" x2="{x0+pw*mx:.1f}" y2="{y0+ph}" class="grid"/><line x1="{x0}" y1="{y0+ph*(1-my/100):.1f}" x2="{x0+pw}" y2="{y0+ph*(1-my/100):.1f}" class="grid"/>',f'<text x="{x0+pw/2}" y="{h-20}" text-anchor="middle" class="label">Measurement Recall Rate</text>',f'<text transform="translate(22 {y0+ph/2}) rotate(-90)" text-anchor="middle" class="label">Evidence Authority Index</text>']
    body += [f'<text x="{x0+pw-10}" y="{y0+20}" text-anchor="end" class="small muted">成熟占位型</text>',f'<text x="{x0+10}" y="{y0+20}" class="small muted">高权威低召回型</text>',f'<text x="{x0+pw-10}" y="{y0+ph-10}" text-anchor="end" class="small muted">主动铺量型</text>',f'<text x="{x0+10}" y="{y0+ph-10}" class="small muted">基础薄弱型</text>']
    pts=[]
    for i,r in enumerate(scores):
        eid=r.get("entity_id","");x=x0+pw*recall.get(eid,0);y=y0+ph*(1-authority.get(eid,0)/100);pts.append((i,x,y))
    labels=_spread_labels(pts)
    for i,r in enumerate(scores):
        eid=r.get("entity_id","");x=x0+pw*recall.get(eid,0);y=y0+ph*(1-authority.get(eid,0)/100);ly=labels.get(i,y-7);tx=min(x+9,w-150);body.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="6" class="bar"/><line x1="{x+5:.1f}" y1="{y:.1f}" x2="{tx-2:.1f}" y2="{ly-3:.1f}" class="grid"/><text x="{tx:.1f}" y="{ly:.1f}" class="small">{esc(r.get("institution",""))}</text>')
...
def heatmap_svg(scores):
    ...
    for i,r in enumerate(rows):
        ...
        for j,(field,maxv,_) in enumerate(DIMENSIONS):
            p=max(0,min(1,num(r.get(field))/maxv));shade=int(245-115*p);fill=f'rgb({shade},{min(255,shade+8)},{min(255,shade+22)})';x=left+j*cellw;body.append(f'<rect x="{x}" y="{y}" width="{cellw-6}" height="30" rx="3" fill="{fill}"/><text x="{x+(cellw-6)/2}" y="{y+20}" text-anchor="middle" class="small">{p*100:.0f}%</text>')
...
def main():
    ...
    if a.self_test:print("generate_charts v2.1.1：请运行 scripts/test_v211.py 完整自测");return 0
