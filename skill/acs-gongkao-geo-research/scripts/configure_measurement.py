...
def configure(run:Path,engines:list[str],context_mode:str,profile:str,repeat_runs:int|None=None,fresh_context:bool|None=None,observation_date:str|None=None,context_isolation_level:str="programmatic",query_variant_mode:str="exact-query-repeat",page_collection_status:str="not-collected"):
    ...
    meta.update({"ai_engine_access_checked":True,"ai_engines_expected":clean,"sampling_mode":infer_sampling(clean),"measurement_profile":profile,"answer_context_mode_expected":context_mode,"repeat_runs_expected":repeats,"fresh_context_required":bool(fresh),"context_isolation_level":context_isolation_level,"query_variant_mode":query_variant_mode,"page_collection_status":page_collection_status,"observation_date":obs,"measurement_contract_status":"configured"})
    mp.write_text(json.dumps(meta,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    config={k:meta[k] for k in ("ai_engine_access_checked","ai_engines_expected","sampling_mode","measurement_profile","answer_context_mode_expected","repeat_runs_expected","fresh_context_required","context_isolation_level","query_variant_mode","page_collection_status","observation_date")}
...
def main():
    ...
    try:print(json.dumps(configure(a.run_dir,a.engine,a.context_mode,a.profile,a.repeat_runs,fresh,a.observation_date,a.context_isolation,a.query_variant_mode,a.page_collection_status),ensure_ascii=False));return 0
