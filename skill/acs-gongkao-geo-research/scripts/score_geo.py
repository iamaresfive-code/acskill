#!/usr/bin/env python3
"""Calculate fixed five-dimension GEO scores; v2.1.1 derives run metrics automatically."""
...
def derive_run_metrics(run_dir:Path)->tuple[dict[str,dict[str,Any]],int]:
    queries=_read_csv(run_dir/"queries.csv");results=_read_csv(run_dir/"query_results.csv");evidence=_read_csv(run_dir/"evidence.csv")
    ...
    temp:dict[str,dict[str,set[str]]]={}
    for r in results:
        ...
        item=temp.setdefault(eid,{"generic":set(),"brand":set()});q=qmap[qid]
    ...
    ev_count=defaultdict(int);ind_domains=defaultdict(set);all_eids=set(temp)
    for e in evidence:
        eid=e.get("entity_id","").strip()
        if not eid:continue
        all_eids.add(eid)
        if e.get("counting_scope","").strip().lower()=="ignored":continue
        ev_count[eid]+=1
        if e.get("independent","").strip().lower() in TRUE_VALUES:
            domain=urlparse(e.get("source_url","")).netloc.lower().removeprefix("www.")
            if domain:ind_domains[eid].add(domain)
    metrics={}
    for eid in all_eids:
        item=temp.get(eid,{"generic":set(),"brand":set()})
        metrics[eid]={"generic_hits":len(item["generic"]),"generic_queries":len(mids),"brand_hits":len(item["brand"]),"evidence_count":ev_count[eid],"independent_domains":len(ind_domains[eid])}
    return metrics,len(mids)

def calculate(payload:Any,metrics=None,denominator=0):
    records=payload if isinstance(payload,list) else [payload];out=[]
    if not all(isinstance(x,dict) for x in records):raise ScoreError("输入必须是 JSON 对象或对象列表")
    for item in records:
        merged=dict(item);eid=str(merged.get("entity_id","")).strip()
        if metrics is not None:
            if not eid:raise ScoreError("使用 --run-dir 时每条记录必须包含 entity_id")
            merged.update(metrics.get(eid,{"generic_hits":0,"generic_queries":denominator,"brand_hits":0,"evidence_count":0,"independent_domains":0}))
...
def self_test():
    ...
    assert score_record(sample)["total"]==76 and tier_for(76)=="A"
...
def main():
    ...
    try:
        if a.self_test:self_test();print("score_geo v2.1.1 自测：通过");return 0
        metrics,den=derive_run_metrics(a.run_dir) if a.run_dir else (None,0)
        print(json.dumps(calculate(load_payload(a.input),metrics,den),ensure_ascii=False,indent=2 if a.pretty else None));return 0
