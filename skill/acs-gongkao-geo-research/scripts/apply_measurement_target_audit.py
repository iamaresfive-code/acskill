...
new_explicit_target in institution/ip/both and non-empty evidence_basis.
"""
...
def apply(run:Path):
    ...
    universe,fields=rcsv(up);audit,_=rcsv(ap);by={r.get("entity_id"):r for r in audit};errors=[]
    ...
    for r in universe:
        ...
        t=(a.get("new_explicit_target") or "").strip();status=(a.get("review_status") or "").strip();basis=(a.get("evidence_basis") or "").strip()
    ...
    wcsv(up,fields,universe)
    if audit:wcsv(ap,af,audit)
    meta=json.loads(mp.read_text(encoding="utf-8"));meta["measurement_target_reviewed"]=True;meta["measurement_target_review_source"]="measurement_target_audit.csv";mp.write_text(json.dumps(meta,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    write_review(run,universe,meta)
    return {"rows":len(universe),"changed":changes,"measurement_target_reviewed":True}
def main():
