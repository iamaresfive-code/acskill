...
def build(run:Path):
    ...
    grouped={k:merge_rows(v) for k,v in groups.items()};obs_merged=merge_rows(observation)
    emergent=[r for r in obs_merged if fnum(r.get("nomination_rate"))>0]
    ...
    if not summaries:
        ...
        if emergent:summaries.append(f"另有 {len(emergent)} 个 Observation 主体在真实 AI 回答中获得提名，已单独披露，避免保守 Universe 过滤掉新竞争者。")
    model={
      ...
      "kpis":{"included_entities":len(included),"national_benchmarks":len(grouped["national_benchmarks"]),"local_institutions":len(grouped["local_institutions"]),"expert_ip":len(grouped["expert_ip"]),"observation_entities":len(observation),"ai_visible_observation_entities":len(emergent),"ai_engines":len(engines),"evidence_count":len(evidence),"definitions":{"included_entities":"经 Market Universe Confirmation 纳入正式研究的全部主体","national_benchmarks":"included 且 market_role=national-benchmark","local_institutions":"included 且 market_scope 为 local/regional 的机构型主体","expert_ip":"included 且 market_role=expert-ip 或 entity_type 为 teacher/ip/expert","observation_entities":"未进入正式比较、但需具名披露的 observation/unresolved 主体","ai_visible_observation_entities":"Stage 1 未纳入主榜、但真实 AI Answer 中至少被提名一次的 Observation 主体"}},
      "executive_summary":summaries,"market_universe":grouped,"observation_group":obs_merged,"ai_visible_observation":emergent,
      ...
      "appendix":{"recheck":{"rows":len(rechecks),"disagreements":len(disagreements),"resolved_disagreements":len(resolved)},"research_assets":[x.name for x in run.iterdir() if x.is_file()],"methodology_notes":["User Seed 只保证研究，不影响任何 AI/资产指标。","Market Scope 不能由 Recall 反推。","AI Answer Measurement 与 Open-Web SERP/页面提及物理分离。","Observation 主体仍保留 AI Metrics；若真实 AI 主动提名，必须在报告中单独披露。","公开网页用于解释 GEO 资产，不替代真实 AI 提名。"]}
    }
