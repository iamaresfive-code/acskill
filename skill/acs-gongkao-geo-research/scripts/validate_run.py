...
VALID_UNIVERSE={"included","observation","excluded","unresolved"}
...
def validate_universe(run:Path,meta:dict,issues:list[Issue],require_confirmed:bool):
    ...
    for i,r in enumerate(universe,2):
        ...
        if (r.get("user_seed") or "").lower() in TRUE:
...
def validate_report(run:Path,issues:list[Issue],universe:list[dict],metrics:list[dict]):
...
def validate(run:Path,strict=False,stage="full"):
    ...
    metrics=[]
    if stage in {"measurement","report","full"}:metrics=validate_measurement(run,meta,issues,ids,mode)
    if stage in {"report","full"}:validate_report(run,issues,universe,metrics)
    return issues
