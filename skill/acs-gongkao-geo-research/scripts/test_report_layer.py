...
Covers empty layers, customer section integrity, developer leakage, customer-unfriendly shorthand,
no-citation semantics, stale letter tiers, unsupported overclaims, and wrong action scope.
"""
...
def swap_report(r:Path,variant:Path,backup_name:str):
    full=r/'deliverables'/'report.docx';backup=r/'deliverables'/backup_name;full.rename(backup);variant.rename(full);return full,backup

def restore_report(full:Path,backup:Path):full.unlink();backup.rename(full)

def assert_injected_failure(r,full,text,expected,idx):
    variant=r/'deliverables'/f'report_probe_{idx}.docx';inject_text(full,variant,text);full2,backup=swap_report(r,variant,f'report_clean_{idx}.docx')
    codes={x.code for x in blocking(val.validate(r,True,'report'))}
    assert expected in codes,f"客户报告负向探针未命中 {expected}: {sorted(codes)}"
    restore_report(full2,backup)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        modeler.build(r);issues=val.validate(r,stage='report');got={x.code for x in errors(issues)}
        assert REQUIRED_CODES<=got,f"空壳 Report Layer 未命中全部强制错误码；实际={sorted(got)}";checks+=1
        assert blocking(issues);checks+=1
        codes2={x.code for x in blocking(val.validate(r,True,'report'))}
        assert "empty-report-section" in codes2 and "empty-analysis" in codes2;checks+=1
        assert not blocking(val.validate(r,True,'measurement'));checks+=1
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
        ...
        except ImportError as e:skips.append(f'Report Layer DOCX 集成测试 SKIP: {type(e).__name__}: {e}')
        else:
            ...
            pos=blocking(val.validate(r,True,'report'));assert not pos,f"完整中文客户报告未通过 report strict：{[(x.code,x.message) for x in pos][:8]}";checks+=1

            full=r/'deliverables'/'report.docx';broken=r/'deliverables'/'report_empty_section.docx'
            strip_section(full,broken,"十一、如何理解本报告");full2,backup=swap_report(r,broken,'report_full.docx')
            codes3={x.code for x in blocking(val.validate(r,True,'report'))};assert "empty-report-section" in codes3;checks+=1;restore_report(full2,backup)

            probes=[
                ("measurement_target=institution","customer-report-internal-field"),
                ("机构题 Top3 率 50%，IP题另算。","customer-report-internal-field"),
                ("本轮引用率恒为 0%。","customer-report-citation-zero"),
                ("资产基础得分 68.9，等级 B+。","customer-report-tier-code"),
                ("GEO 资产基础与 AI 可见度仅弱相关，更依赖品牌记忆。","customer-report-overclaim"),
                ("为全部 12 个本土机构建立页面，作为工程有效性的唯一判据。","customer-report-action-scope"),
            ]
            for i,(txt,code) in enumerate(probes,1):assert_injected_failure(r,full,txt,code,i);checks+=1
