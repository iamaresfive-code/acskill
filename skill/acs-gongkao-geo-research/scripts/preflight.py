#!/usr/bin/env python3
"""Create/validate GEO v2.1.1 preflight metadata without starting research."""
...
def normalize_formats(values:list[str])->list[str]:
    out=[]
    for value in values:
        fmt=FORMATS.get(value.strip().lower())
...
def make_metadata(region:str|None,entities:list[str]|None,formats:list[str]|None,entities_decided:bool=False)->dict[str,object]:
    meta={"schema_version":"2.1.1","skill_version":"2.1.1"}
    if region:
    ...
    meta.update({"candidate_pool_frozen":False,"semantic_coverage_gate":False,"saturation_gate":False,"local_ecosystem_gate":False})
    return meta
...
def self_test():
    assert missing_questions(make_metadata(None,None,None))==["region"]
    ...
    e=make_metadata("广东",[],["PDF"],entities_decided=True);assert e["run_status"]=="ready" and e["local_ecosystem_gate"] is False


def main()->int:
    p=argparse.ArgumentParser(description="生成 GEO v2.1.1 Preflight metadata")
    ...
    if args.self_test:self_test();print("preflight v2.1.1 自测：通过（5 个场景）");return 0
