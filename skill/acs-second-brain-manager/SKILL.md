...
安装名保留 `acs-second-brain-manager`，以免已有安装地址失效。v2.0将长期管理器改为治理安装器：落地后的规则与工具独立属于目标库，不依赖本Skill常驻。
...
- `validate_profile.py` 和旧YAML模板仅兼容v1已有配置，不作为v2安装前提；v2只需Python 3.10+标准库。
